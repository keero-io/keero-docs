...
```mermaid
flowchart LR
  A["ESP32-S3 Mainboard Core"] --> B["Interaction Layer"]
  A --> C["Power and Portable Operation"]
  A --> D["Module and Dock Expansion"]
  E["Firmware Layer"] --> A
  B --> F["Camera"]
  B --> G["Audio"]
  B --> H["Display"]
  B --> I["Haptics and Sensors"]
  D --> J["Dock"]
  D --> K["Tracks"]
  D --> L["Future Accessories"]
```
